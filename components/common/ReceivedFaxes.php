...
setStore('recieved-info', $default_data);

if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $actions = $_POST["actions"];

    if ($actions == 'btnReceivedInfo') {
        $sql_query_rec = "SELECT * FROM faxes WHERE id = '" . request("txt-id") . "'";
        $rec_info = mysqli_query($db_connection, $sql_query_rec);
        if ($rec_info) {
            updateStore('recieved-info', 'info', $rec_info->fetch_assoc());
            router("received-faxes-viewer");
        }
    }
}

$sql_query_faxes = "SELECT * FROM faxes WHERE type = 'Received' ORDER BY createdAt DESC";
$faxes = mysqli_query($db_connection, $sql_query_faxes);


function formatDateWithSept($date, $format) {
    $formattedDate = date($format, strtotime($date));
    if (strpos($formattedDate, 'Sep') !== false) {
        $formattedDate = str_replace('Sep', 'Sept', $formattedDate);
    }
...
<div id="component-body">
    <div class="pdf-area">
        ...
        ?>
            <div class="upper">
                <p><b><?php display(formatDateWithSept($row["createdAt"], "M jS")); ?></b>&nbsp; <?php display(formatDateWithSept($row["createdAt"], "g:ia")); ?></p>
                <p><b><?php display($row["_to"]); ?></b></p>
            </div>
            <div class="head">
                <div class="det">
                    <p class="pd-lft">Fax Receive Successful</p>
                ...
                <form action="received-faxes" method="post">
                    <div class="options" id="option-<?php display($row["id"]); ?>">
                        <div class="sub-menu">
                            <div class="select">
                                <button type="submit" name="actions" value="btnReceivedInfo">
                                    <div class="icon">
                                        <figure>
